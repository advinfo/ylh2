...
//var_dump($extAttr);
     ...
     * @param $chnId
     */
    public function rankingJson($chnId){
	    $data=array(array("rank"=>"土豪榜","list"=>array(array("name"=>"李吉吉","value"=>"986543积分"),array("name"=>"张三丰","value"=>"88883积分"),
                            array("name"=>"陈嘉琦","value"=>"65883积分"),array("name"=>"李建新","value"=>"21883积分")
                        )),
                array("rank"=>"幸运榜","list"=>array(array("name"=>"酷家乐","value"=>"8899金币"),array("name"=>"张三丰","value"=>"8811金币"))),
                array("rank"=>"分享榜","list"=>array(array("name"=>"肯尼迪","value"=>"9865人"),array("name"=>"欧阳芳","value"=>"88人"),
                            array("name"=>"张家口","value"=>"78人"))
                        ),
            );
    ...
    public function userInfoJson(){
        ...
        //unset($info['rightInfo']);
//dump($info);
        $rtArr=array();
        try{
            if(null==$info) throw new Exception('用户未登录.');
            //基本信息
            $rtArr['userName']=$info['userName'];
            $rtArr['account']=$info['account'];
            $rtArr['userlevel']=$info['userlevel'];
            $rtArr['viplevel']=$info['viplevel'];
            $rtArr['experience']=$info['experience'];

            $rtArr['image']="/Public/images/unknow1.png";
            $rtArr['bozhu']=$info['userExtAttr']['bozhu'];
            $rtArr['phone']=$info['userExtAttr']['phone'];
            //第三方平台信息
            $dbtpu=D('thirdpartyuser');
//$info['userId']=5200;
            $rec=$dbtpu->where('userid='.intval($info['userId']))->select();
//echo $dbtpu->getLastSql();
            $rtArr['partys']=array();
            foreach ($rec as $val){
                array_push($rtArr['partys'],array('party'=>$val['party'],
                    'attr'=>array('nick'=>$val['nickname'],'mobile'=>$val['mobile'],'rcm'=>$val['rcm_id'])));
            }
