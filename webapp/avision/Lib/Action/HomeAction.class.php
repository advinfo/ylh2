...
//dump($_SESSION['userinfo']);	
    ...
    public function index($type=0){
    	$scrType=$this->getScreenType();
    	$auth=new authorize();
logfile("register:after author:".$_SESSION['userinfo']['account'],LogLevel::DEBUG." sessionid".session_id());
...
//dump($_SESSION);dump($_REQUEST); die();
    	$this->baseAssign();
...
logfile('acceptUrl:'.$acceptUrl,LogLevel::DEBUG);
    ...
    public function authenJson($account,$password,$accept=''){
        ...
        $ret=$this->auth->issue($account,$password);
        if($ret){
            //登录认证成功
