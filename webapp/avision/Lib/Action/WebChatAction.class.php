...
class WebChatAction extends SafeAction {
	...
	public function IsCanChat($chnId = 0)
	{
		...
		if($userId<1 || C('anonymousUserId') == $userId)	{
			//匿名用户不能发言
			$ret = false;
		}
		else	{
			$dal = D('channelnochat');
			$w = array('chnid'=>$chnId, 'uid'=>$userId);
...
//echo $result[0][id];			
     ...
     * 发送新信息
     */
    public function newMsgJson($chnid=0,$message=''){

        try{
            if($chnid<1) throw new Exception("缺乏频道信息.");
            if(!$this->IsCanChat($chnid)) throw new Exception("您目前不能发言。");
            $dbChat=D('webchat');
            $record=array('message'=>$message,'senderid'=>$this->userId(),'sendername'=>$this->userName(),
                'userlevel'=>$this->getUserInfo('userlevel'),'viplevel'=>$this->getUserInfo('viplevel'),'chnid'=>$chnid);
            $result=$dbChat->add($record);
//echo $dbChat->getLastSql();
            if($result<1) throw new Exception("插入信息失败");
            $record=$dbChat->getNewMsg($chnid);
            $output=array('chat'=>$record);
        }catch (Exception $e){
