...
//echo "call HDPlayerAction/play<br>";
	...
	public function getPlayUri(){
		...
		try{
			if(null==$streamName) throw new Exception('Parameter was request: stream.');
			...
			if(1>$streamId) throw new Exception('I do not know this stream:'.$streamName);
	...
	public function getVodUri(){
		...
		$hls = '';
		try{
			if(null==$vodid) throw new Exception('Parameter was request: stream.');
			...
			if(''==$hls) throw new Exception('Stream configer error');
            $retAttr=array('success'=>'true','uri'=>$hls);//为了success是第一个属性
            //echo "callback(".json_encode($retAttr,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE).")";
			Oajax::successReturn(array('uri'=>$hls));
		}catch (Exception $e){
			Oajax::errorReturn($e->getMessage());
		}
